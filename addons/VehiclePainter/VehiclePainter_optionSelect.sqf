...
storePurchaseHandle = player spawn
{
	...
	_vehiclePainter =
	{
		...
		{
			_text = format ["Dinero insuficiente! Necesitas $%1 para personalizar tu vehículo.",_price];
		...
		if (_price < _playerMoney) then
		{
			...
			_text = format ["Has pagado $%1 para personalizar tu vehículo.",_price];
			[_text, 10] call mf_notify_client;
		...
		{
			_text = format ["La compra de esta personalización del vehículo es excusiva de cuentas VIP.",_price];
			[_text, 10] call mf_notify_client;
			playSound "FD_CP_Not_Clear_F";
		};
