...
/* argument: Keep Autonomus unlocked and Autonomus disabled until */
...
private _isOwner = (_manager getVariable ["ownerUID", ""]) == getPlayerUID player;
...
then     
{
	...
	if (_distance > _BaseRadius) then										// Check distance from player base, kinda.
	{
		...
		["Please, lock down this inside your own base only.", 5] call mf_notify_client; // Let him know
	}
	else
	{
		if (_distance <= _BaseRadius && _isOwner)  then									// Check if from player is in his base
		{
