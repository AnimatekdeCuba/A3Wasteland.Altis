...
if (!isNil "storePurchaseHandle" && {typeName storePurchaseHandle == "SCRIPT"} && {!scriptDone storePurchaseHandle}) exitWith {hint "Espere, por favor, su compra previa está siendo procesada."};
...
storePurchaseHandle = _this spawn
{
	...
	{
		_itemText = _this select 0;
		hint parseText format ["No tienes suficiente dinero para un<br/>""%1""<br/>Lo siento", _itemText];
		playSound "FD_CP_Not_Clear_F";
		_price = -1;
	};
