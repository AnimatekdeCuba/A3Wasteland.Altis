	...
	switch (_territoryAction) do
	{
		case "CAPTURE":
		{
			...
			if (_territoryCaptureCountdown > 60) then
			{
				...
				_territoryActionText = format["Capturando territorio en <t font='%1'>%2 minutos</t>", _boldFont, _minutes + 1];
			}
			else
			{
				if (_territoryCaptureCountdown < 5) then
				{
					_territoryActionText = "Transición de territorio en progreso...";
				}
				else
				{
		...
		case "CaptureDefender":
		{
			...
			if (_territoryCaptureCountdown > 60) then
			{
				...
				_territoryActionText = format["Enemigo capturando su territorio en <t font='%1'>%2 minutos</t>", _boldFont, _minutes + 1];
			}
			else
			{
				if (_territoryCaptureCountdown < 5) then
				...
				{
					_territoryActionText = format["Enemigo capturando su territorio en <t font='%1'>%2 segundos</t>", _boldFont, _territoryCaptureCountdown];
				};
			};
		};
		case "BLOCKEDATTACKER": { _territoryActionText = "Captura de territorio bloqueada" };
		case "BLOCKEDDEFENDER": { _territoryActionText = "Territorio bajo ataque" };
		case "RESET":           { _territoryActionText = "Captura de territorio iniciada" };
		case "ResetDefender": { _territoryActionText = "ALERTA! Invasión enemiga al territorio capturado" };
	};
...
while {true} do
{
	...
	if (_survivalSystem) then {
		_strArray pushBack format ["%1 <img size='0.7' image='client\icons\water.paa'/>", round (player getVariable ["acex_field_rations_thirst", 0])];
		_strArray pushBack format ["%1 <img size='0.7' image='client\icons\food.paa'/>", round (player getVariable ["acex_field_rations_hunger", 0])];
	};
