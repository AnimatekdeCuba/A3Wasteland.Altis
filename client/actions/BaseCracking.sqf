...
// Setup Variables
	...
	// aditional condition
	/*private _class = typeOf _crackervehicle;
	...
	private _animPhases = (configProperties [_vehCfg >> "AnimationSources", "getText (_x >> 'displayName') != '' && {getNumber (_x >> 'scope') > 1 || !isNumber (_x >> 'scope')}"])
	                      apply { [configName _x, [0,1] select (_crackervehicle animationPhase configName _x >= 1)] };*/
...
// Start Hack Action
	...
	if (_cracker iskindof _crackervehicle) then
	{
		...
		if (_operatorcheck == 0) then
		{
			...
			{
				/*if (_animPhases = ["mast_handler", 0]) then
				{*/
					if (_playerUID == _managerOwner) then
					{
						titletext ["La Base más cercana te pertenece. Hackeo cancelado!", "PLAIN DOWN"];
					} else
					{
						...
						while {HackInProgress} do
						{
							...
							//Set up the text
							_txt1 = format ["Hackeando. Tiempo restante: %1:%2:%3", _TimeRemainingTotalHrs, _TimeRemainingTotalMins, _TimeRemainingTotalSecs];
				...
				/*} else 
				{
					titletext ["Debes deplegar el Mastil", "PLAIN DOWN"]
				};*/
		...
		{
			titletext ["Debes estar en el asiento de pasajero para operar el Sistema de Hackeo", "PLAIN DOWN"];
		};
	// Let the player know they're no in the correct vehicle
	}else
	{
		titletext ["Este vehículo no contiene un Sistema de Hackeo de Bases", "PLAIN DOWN"];
	};
