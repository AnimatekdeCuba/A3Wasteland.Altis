/*
Author: GMG_Monkey edited by Animatek
Filename: BaseCracking.sqf
Purpose: Allow player to hack into enemy base manager remotely
*/
// Setup Variables
	private _antennaExtended = false;

	// define event handler to detect when the vehicle's animation state changes
	rhs_gaz66_r142_msv addEventHandler ["AnimChanged", {
	    _animState = _this select 1;
	    if (_animState isEqualTo "rhs_gaz66_r142_msv_antenna_up") then {
	        _antennaExtended = true;
	    } else {
	        _antennaExtended = false;
	    }
	}];

	// function to return whether the antennas are extended
	fn_isAntennaExtended = {
	    _antennaExtended
	};
	//Initialize Hack in progress flag
	HackInProgress = false;
	// What vehicle is to be used
	private _crackervehicle = "rhs_gaz66_r142_msv"; // "O_Truck_03_device_F";
	// Initialize time to hack variable
	private _hacktime = (60* 60 *1.5);//* 60 *1.5
...
// Start Hack Action
	...
	if (_cracker iskindof _crackervehicle) then
	{
		...
		if (_operatorcheck == 0) then
		{
			...
			{
				if (rhs_gaz66_r142_msv call fn_isAntennaExtended) then  // check if the antennas are extended
				{
					if (_playerUID == _managerOwner) then
					{
						...
						private _Hack = [_hacktime, _manager, _failtext] spawn
						{
							...
							if !((vehicle player) iskindof 'rhs_gaz66_r142_msv') exitwith {titletext [format ["%1",_failtext], "PLAIN DOWN"]; HackInProgress = false};
							...
							sleep (_hacktime / 10);
							if !((vehicle player) iskindof 'rhs_gaz66_r142_msv') exitwith {titletext [format ["%1",_failtext], "PLAIN DOWN"]; HackInProgress = false};
							titletext ["Hack Successfull, password reset to 1234. Buen día!", "PLAIN DOWN"];
			...
			{	 // Let the player know the antennas are not extended
    			titletext ["You must extend the antennas before hacking", "PLAIN DOWN"]
			};
